...
**结论**：万级 SKU 以内可以直接用默认配置；到十万级就要动下面的旋钮。

### 怎么省

**旋钮一：`HISTORY_FOCUS_ONLY`（省得最多）**

历史的**唯一**用途是算基线，而长尾 SKU 本来就交易少、告警少，存它的历史收益很低 ——
但它占了容量的绝大部分，因为长尾在数量上是大头。

```
HISTORY_FOCUS_ONLY=true
FOCUS_SKUS=SKU-A,SKU-B,SKU-C
```

开启后只记录 `FOCUS_SKUS` 里的 SKU。

⚠️ **降级方向必须清楚**：长尾 SKU 不存历史 → 查不到基线 → 样本不足 →
**一律放行**，行为退回纯环比。也就是说代价是"可能多报"，**不会漏报**。
这跟"为了省空间把告警吃掉"是两回事，后者是绝不能接受的。

⚠️ 开了开关却没配 `FOCUS_SKUS` = 一条历史都不记，基线完全失效。
这种情况下会打一条 WARNING 明确告诉你。

**旋钮二：`HISTORY_RETENTION_DAYS`**

90 → 30 天，占用直接砍到 1/3。但基线是"最近 N 天同一时段"，
保留期短了样本就少，必要时把 `BASELINE_LOOKBACK_DAYS` 一起调小，
否则样本凑不够、基线等于没开。

清理是自动的：每天凌晨那轮 `monitor` 会顺带 `prune()`（不用单独配 crontab），
并 `VACUUM` 回收空间 —— 不做的话 db 文件只增不减。
